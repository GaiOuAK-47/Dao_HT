...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["action"] == "add") { 
    ...
    try {
        foreach ($document as $i => $v) { 
            ...
            $attname = $v["attname"];
            $attno = $i + 1;
            $sql = "INSERT INTO receiptattachment(recode,attno,attname,url) VALUES (?,?,?,?)";
            $stmt = $conn->prepare($sql); // prepare
            $stmt->bind_param('siss', $recode, $attno, $attname, $url); // bind array at once
            if (!$stmt->execute()) throw new Exception("Insert data error.");
        }
        $query = mysqli_query($conn,"select 1 from receiptattachment where recode = '$recode' limit 1");
        if(!!!$query)  throw new mysqli_sql_exception("insert document fail.");

        $conn->commit();  
        http_response_code(200);
        echo json_encode(array('status' => '1', 'message' => "อัพโหลดไฟล์สำหรับ PO $recode สำเร็จ"));
    ...
    try {
        ...
        if (!empty($file_deleted)) {
            foreach ($file_deleted as $i => $v) {
                ...
                $sql  = "DELETE FROM receiptattachment WHERE code = ?";
                $stmt = $conn->prepare($sql); // prepare
        ...
        if (!empty($file_data)) {
            foreach ($file_data as $i => $v) {
                ...
                $sql = "UPDATE receiptattachment SET attname = ?, url = ?  WHERE code = ?"; // sql 
                $stmt = $conn->prepare($sql); // prepare
        ...
        if (!empty($_FILES["file"])) {
            $fileData = json_decode($_POST["fileData"], true);
            $file = $_FILES["file"];
            $sql = "select max(attno) m from receiptattachment where recode = '$recode' ";
        ...
        $conn->commit();  
        http_response_code(200);
        echo json_encode(array('status' => '1', 'message' => "อัพเดทไฟล์สำหรับ Receipt code $recode สำเร็จ"));
    } catch (mysqli_sql_exception $e) {
        $conn->rollback(); 
    ...
    try {
        $recode = $_GET["c"];
        $sql = "SELECT * from receiptattachment a where recode = '$recode';"; 
        $query = mysqli_query($conn,$sql);
        // Fetch all
